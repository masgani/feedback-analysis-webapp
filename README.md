...
本アプリケーションでは、デバッグおよび動作確認を目的として  
標準の logging モジュールによるログ出力を行っています。
...
ログはデフォルトで Docker コンテナの標準出力（docker compose up 実行時のターミナル）に出力されます。
必要に応じて、環境変数によりファイル出力へ切り替えることも可能です。